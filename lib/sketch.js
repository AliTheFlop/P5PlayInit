...
function spawnEnemy() {
    let enemy = createSprite(width + 20, random(50, height - 50), 30, 30);

    // Randomly decide if this is a purple enemy (20% chance)
    if (random() < 0.2) {
        enemy.shapeColor = color("purple");
        enemy.velocity.x = -random(3, 5 + level); // Purple enemies are faster
        enemy.health = 3; // Purple enemies take more hits
    } else {
        enemy.shapeColor = color("red");
        enemy.velocity.x = -random(2, 4 + level);
        enemy.health = 2;
    }
...
function shoot() {
    if (tripleShootPowerActive) {
        ...
        bulletStraight.shapeColor = color("green");
        ...
        bulletUp.shapeColor = color("green");
        ...
        bulletDown.shapeColor = color("green");
